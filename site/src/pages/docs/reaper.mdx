...
<Command name='/reap'>
	Reap the time since the last reap! If you have a free reap and you use this
	command when your cooldown hasn't expired or you were the last one to reap,
	you will use up a free reap.
</Command>
<Command name='/reaper leaderboard [gameid]'>
	Returns an embed with a pretty-printed leaderboard of the top 20 participants.
	`[gameid]` is optional and assumed to be the round number of the currently
	active game.
</Command>
<Command name='/reaper when'>
	Shows when your cooldown expires and how many free reaps you have.
</Command>
<Command name='/reaper last'>Shows who the last person to reap was.</Command>
<Command name='/reaper score [user] [gameid]'>
	Returns the score and rank for a user in reaper round `[gameid]` on that
	channel.
</Command>
...
<Command name='/reaper init [win] [cooldown]' admin={true}>
	Create a new reaper round on a channel. `[win]` seconds to win and
